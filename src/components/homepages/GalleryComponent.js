...
function GalleryComponent({ formik, item, value, edit, dropdown_options }) {
  console.log("VALUES", value, item);
  const [menuType, setMenuType] = useState(null);
  return (
    <div className="row">
      <div className="p-2 mt-2 mb-2 bg-light">
        <p className="font-weight-bold">Gallery Component</p>
      </div>
              ...
              {arrayValues.map((single, fieldIndex) => (
                <div key={fieldIndex} className="row">
                  ...
                  <div className="col-md-6">
                    <FileUpload
                      title={"Image Tablet"}
                      item={`${item}[${fieldIndex}].imageTablet`}
                      formik={formik}
                      value={value[fieldIndex]?.imageTablet}
                    />
                  </div>
                  <div className="col-md-6">
                    <FileUpload
                      title={"Image Mobile"}
                      item={`${item}[${fieldIndex}].imageMobile`}
                      formik={formik}
                      value={value[fieldIndex]?.imageMobile}
                    />
                  </div>
                  <div className="col-md-6">
                    <TextInput
